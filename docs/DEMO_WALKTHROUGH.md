# Demo walkthrough: goal → discovery → artifact → replay → error → escalation → chatbot → guardrails
...
in `/README.md`, `/REPORT.md`, and `/evidence/INDEX.md`. §§1–5 walk the original CLI-driven
discover → artifact → replay → error → escalation slice; §§6–7 cover the newer goal-driven
chatbot entry point and its guardrails, added in later phases (`TASKS.md` has the full log).
...
## 3. Deterministic replay — success
...
## 4. Replay hitting an error / exceptional state
...
## 6. The newer entry point: goal-driven chatbot

Everything above drives the CLI directly against one named capability. The admin console
(`GET /admin`) adds a chatbot-style entry point — type a goal in plain English, no capability id,
no inputs form — that routes through the full `AgentOrchestrator` pipeline (Intent Analyzer →
Planner → PlanValidator → CapabilityResolver → Executor → `GroundedSynthesizer`,
...
## 7. Guardrails — refused or clarified before any browser action
